...
def add_remaining_useful_life(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    # TODO 1:
    # Create the remaining useful life target, just like in the leaky model.
    #
    # Hint:
    # - Group rows by engine.
    # - Find the final recorded cycle for each engine.
    # - Subtract the current cycle from that final cycle.
...
def feature_columns(frame: pd.DataFrame) -> list[str]:
    # TODO 2:
    # Remove the leaking identifier column from the model inputs. Keep `cycle`;
    # the current cycle is known when the model is used.
    #
    # Strong hint:
    #   blocked_columns = {"engine", TARGET}
    blocked_columns = {TARGET}
    if "engine" not in blocked_columns:
        raise NotImplementedError("TODO 2: remove `engine` from the input features.")
